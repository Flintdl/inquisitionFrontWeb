...
const startGame = (io, roomName, existingRoom) => {
  ...
  if (!existingRoom.startGame && users.length >= 3) {
    existingRoom.startGame = true; // Set the flag to indicate that the game is now in progress
    let currentPlayerIndex = 0;
      ...
      const countdownInterval = setInterval(() => {
        ...
        if (countdown <= 0) {
          ...
          if (currentPlayerIndex < users.length) {
            setTimeout(playTurn, 1000); // Delay before starting the next turn
          } else {
            // All turns are done
            io.to(roomName).emit('game_over');
            gameInProgress = false; // Reset the flag when the game is over
          }
        } else {
          countdown--;
        }
